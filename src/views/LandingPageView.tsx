  ...
  return <div className="min-h-screen bg-[#F9F9F7] dark:bg-[#141412] text-[#1A1A1A] dark:text-white text-start">
    ...
    <main>
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-16 sm:py-24 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <p className="text-emerald-700 dark:text-emerald-400 font-semibold mb-5">{tr('أثر | إدارة التأجير وسجل المركبة','ATHAR | Rental management and vehicle history')}</p>
        ...
        <div className="rounded-3xl bg-[#1b2422] p-6 sm:p-10 text-white shadow-xl">
          <p className="text-emerald-300 text-sm mb-6">{tr('رحلة المركبة في أثر','The vehicle’s journey in Athar')}</p>
          {[
          ...
          ].map(([title,body],i)=><div key={title} className="flex gap-4 py-6 border-t border-white/15"><span className="text-emerald-300 font-mono">0{i+1}</span><div><h2 className="text-xl font-semibold">{title}</h2><p className="mt-2 text-neutral-300 leading-7">{body}</p></div></div>)}
      ...
      <section id="about" className="border-y border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#1B1B18] scroll-mt-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-16 grid md:grid-cols-2 gap-10">
          <div><p className="text-emerald-700 dark:text-emerald-400 font-semibold mb-3">{tr('من نحن','About us')}</p><h2 className="text-3xl font-bold leading-relaxed">{tr('فريق أثر. نبدأ من عمل مكاتب التأجير اليومي.','The Athar team. Starting with the everyday work of rental offices.')}</h2></div>
      ...
      ].map(([title,body])=><div key={title}><h3 className="text-xl font-semibold mb-3">{title}</h3><p className="leading-8 text-emerald-100">{body}</p></div>)}</div></div></section>
