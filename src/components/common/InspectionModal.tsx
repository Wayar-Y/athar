...
export const InspectionModal:React.FC=()=>{
 ...
 return <dialog ref={dialog} onCancel={()=>setIsInspectionModalOpen(false)} className="m-auto p-0 w-[min(95vw,650px)] max-h-[90vh] rounded-2xl bg-white dark:bg-[#1b1b18] text-neutral-900 dark:text-white backdrop:bg-black/60" aria-labelledby="inspection-title">
  ...
  <form className="p-5 space-y-4" onSubmit={e=>{e.preventDefault();if(!vehicle)return;try{
   ...
   <label className="block">{tr('نوع الفحص','Inspection type')}<select className="athar-field" value={type} onChange={e=>setType(e.target.value as InspectionRecord['type'])}><option value="routine">{tr('دوري','Routine')}</option><option value="before_rental">{tr('تسليم','Handoff')}</option><option value="post_rental">{tr('إرجاع','Return')}</option></select></label>
   ...
   ].map(f=><label key={f.key}>{f.label}<select className="athar-field" value={condition[f.key as keyof typeof condition]} onChange={e=>setCondition({...condition,[f.key]:e.target.value})}>{f.options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>)}</div>
